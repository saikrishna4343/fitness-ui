...
- **Three providers behind one seam** (`providers.ts`): `COACH_MODEL` picks the model and
  therefore the provider — `claude*` goes to `anthropic.ts`, `gpt*`/`o<digit>*` to
  `openai.ts` (the default, `gpt-5-mini`), anything else to `gemini.ts`. OpenAI uses
  stateless Chat Completions, does not stream, takes the tool schemas unsimplified, and
  runs at `reasoning_effort: 'low'` because thinking bills as output. What makes the swap cheap is that replay is text-only, so no provider's
